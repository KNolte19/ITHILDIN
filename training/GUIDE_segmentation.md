...
- [ ] Run the image processing script to align and normalize images:
    ```bash
    cd training
    python generate_segmentation_data.py --experiment my_dataset --process-only
    ```
...
### Missing Masks/Annotations
- [ ] Every processed image has a corresponding mask in `segmentation_fromannotator/`
